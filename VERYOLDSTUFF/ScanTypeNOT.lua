...
function ScanTypeNOT.injectAAScript()
  ...
  if not (CEVer>=6.4 and CEVer<=6.6) then
    print('ScanTypeNOT script not injected, wrong CE version')
    return false
  end

  if     CEVer==6.4  then VariableTypeOffset=is64bit and '1D9' or '131'
  elseif CEVer==6.5  then VariableTypeOffset=is64bit and '1F9' or '149'
  elseif CEVer==6.51 then VariableTypeOffset=is64bit and '230' or '149'
  elseif CEVer==6.6  then VariableTypeOffset=is64bit and '1F9' or '14D'
                     end

  local script,AOB_inject_scantypenot='',''

  if is64bit then -- now handle 64bit versions,

    if CEVer==6.51 then AOB_inject_scantypenot=[[48 8B 45 F8 48 8D 88 E0 00 00 00 48]]
                   else AOB_inject_scantypenot=[[48 8B 45 F8 48 8D 88 00 01 00 00 48]] end

    -- it works for 64bit CE6.4, 6.5, 6.5.1, 6.6
    script = [[
aobscanmodule(address,cheatengine-x86_64.exe,]]..AOB_inject_scantypenot..[[)
...
return:]]
    ...
    if CEVer==6.4 then AOB_inject_scantypenot=[[8B 55 FC 8D 8A A0 00 00 00]]     -- CE6.4
                  else AOB_inject_scantypenot=[[8B 45 FC 05 A8 00 00 00 8B]] end -- CE6.5, 6.5.1, 6.6

    script = [[
aobscanmodule(address,cheatengine-i386.exe,]]..AOB_inject_scantypenot..[[)//
alloc(newmem,$1000)
label(code)
label(return)
label(OrigCheckRoutine)
...
return:]]
  ...
  if not ScanTypeNOT.InjectedAAScript then print('ScanTypeNOT script not injected') end
